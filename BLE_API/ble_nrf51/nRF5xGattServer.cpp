...
//            BLE_ERROR_PARAM_OUT_OF_RANGE);
    //krix
    ASSERT( ERROR_NONE == sd_ble_gatts_value_get(attributeHandle, 0, lengthP, buffer),BLE_ERROR_PARAM_OUT_OF_RANGE);
...
//                    BLE_ERROR_PARAM_OUT_OF_RANGE );
    	//krix
        ASSERT_INT( ERROR_NONE,
                  	  sd_ble_gatts_value_set(attributeHandle, 0, &len, buffer),
					  BLE_ERROR_PARAM_OUT_OF_RANGE );
        return BLE_ERROR_NONE;
    }
...
//                                BLE_ERROR_PARAM_OUT_OF_RANGE );
                	//krix
                	ASSERT_INT(ERROR_NONE,
                				sd_ble_gatts_value_set(attributeHandle, 0, &len,
								buffer), BLE_ERROR_PARAM_OUT_OF_RANGE);
        ...
        //krix uint32_t err = sd_ble_gatts_value_set(connectionHandle, attributeHandle, &value);
        uint32_t err = sd_ble_gatts_value_set(attributeHandle, 0, &len,buffer);
        switch(err) {
            case NRF_SUCCESS:
...
void nRF5xGattServer::hwCallback(ble_evt_t *p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id) {
        ...
        case BLE_GATTS_EVT_SYS_ATTR_MISSING:
            sd_ble_gatts_sys_attr_set(gattsEventP->conn_handle, NULL, 0);//krix, 0);
            return;
